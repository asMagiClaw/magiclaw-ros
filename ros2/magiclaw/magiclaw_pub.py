...
class MagiClawPublisher(Node):
    """
    A ROS node that publishes the state of the MagiClaw robot.

    This node subscribes to messages from a ZMQ server and publishes them to the
    `/magiclaw/state` topic in ROS.
    """

    def __init__(self, host: str, port: int, rate: int) -> None:
        """
        Initialize the MagiClawPublisher node.

        Args:
            host (str): The host address of the ZMQ server.
            port (int): The port number of the ZMQ server.
        """

        super().__init__("magiclaw_publisher")
        self.sub = MagiClawSubscriber(host=host, port=port)
        self.pub = self.create_publisher(MagiClaw, "/magiclaw/state", 10)
        self.timer = self.create_timer(1.0 / rate, self.publish_message)

    def publish_message(self):
        """
        Callback function to receive and publish messages.
        """
        try:
            ...
            self.get_logger().warn(f"Failed to receive/convert message: {str(e)}")
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description="MagiClaw Publisher for ROS")
    parser.add_argument(
        "--host", type=str, required=True, help="Host address of the ZMQ server"
    )
    parser.add_argument(
        "--port", type=int, required=True, help="Port number of the ZMQ server"
    )
    parser.add_argument(
        "--rate", type=int, default=30, help="Publishing rate in Hz (default: 30)"
    )
    args = parser.parse_args()

    # Initialize the MagiClawPublisher node
    node = MagiClawPublisher(host=args.host, port=args.port, rate=args.rate)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
